...
instance Builtin.HasDefinitions StarterContracts where
    getDefinitions = [UnsigExchange]
    getSchema =  \case
        UnsigExchange -> Builtin.endpointsToSchemas @UnsigExchange.UnsigExchangeSchema
    getContract = \case
        UnsigExchange -> SomeBuiltin (Issuer.endpoints @ContractError)
